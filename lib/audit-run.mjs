...
import { VENDOR_PATH } from '../guards/tamper.mjs'
import { findBinary } from './tools.mjs'
...
const PHP_PATHS = ['app', 'src', 'database', 'routes', 'lib', 'config']
...
function auditNpm(component) {
  const lock = ['package-lock.json', 'npm-shrinkwrap.json'].some((f) => existsSync(join(component.dir, f)))
  if (!lock) {
    const other = ['pnpm-lock.yaml', 'yarn.lock'].find((f) => existsSync(join(component.dir, f)))
    return {
      available: false,
      hint: other ? `${other} — "npm audit" passt hier nicht, bitte "${other.startsWith('pnpm') ? 'pnpm' : 'yarn'} audit" von Hand` : 'kein Lockfile',
    }
  }
  const run = capture('npm', ['audit', '--json'], { cwd: component.dir })
  const json = parseJsonLoose(run.stdout)
  if (!json) return { available: false, hint: 'npm audit lieferte kein JSON (offline?)' }
  return { available: true, counts: parseNpmAudit(json) }
}
...
const SOURCE_EXT = /\.(php|[jt]sx?|mjs|cjs|vue)$/i
const TEST_PATH = /(^|\/)(tests?|__tests__|spec)\/|\.(test|spec)\.[mc]?[jt]sx?$|Test\.php$/i
...
function collectHygiene(root, component, ignorePaths = []) {
  ...
  for (const path of trackedFiles(root, component.path)) {
    if (!SOURCE_EXT.test(path) || VENDOR_PATH.test(path)) continue
...
export async function runAudit(root, { quiet = false, secrets = true } = {}) {
  ...
  if (detected.length === 0) {
    return { root, repo: repoFacts(root), components: [], recommendation: null }
  }
  ...
  return { root, repo, components, recommendation }
}
