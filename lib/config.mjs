...
export const KNOWN_STACKS = ['laravel', 'php', 'react-ts', 'next-ts', 'node-ts', 'python']

/**
 * Konfigurationen, deren Einbindung `quality audit` prüft (lib/wiring.mjs).
 * Der einzige Wert ist `own`: „diese Konfiguration ist bewusst eigen, schweig
 * darüber". Ein Projekt mit begründet eigenem Stil soll den Hinweis dauerhaft
 * abstellen können — ein Hinweis, den man nicht abstellen kann, wird nach dem
 * zweiten Mal überlesen, und dann übersieht man auch den nächsten.
 */
export const WIRING_KEYS = ['phpstan', 'prettier', 'tsconfig', 'ruff']
const WIRING_VALUES = ['own']
...
export function validateConfig(raw, file) {
  ...
  if (configs === null || typeof configs !== 'object' || Array.isArray(configs)) {
    throw new ConfigError(`${file}: configs muss eine Map sein, z.B. configs: { prettier: own }.`)
  }
  for (const [key, value] of Object.entries(configs)) {
    if (!WIRING_KEYS.includes(key)) {
      throw new ConfigError(
        `${file}: configs.${key} ist unbekannt, bekannt sind: ${WIRING_KEYS.join(', ')}.`
      )
    }
    if (!WIRING_VALUES.includes(value)) {
      throw new ConfigError(
        `${file}: configs.${key} = "${value}" ist unbekannt, erlaubt ist ${WIRING_VALUES.join(' oder ')}.`
      )
    }
  }

  // Pfade, die der Tamper-Check nicht bewertet. Nötig z.B. für das Framework
  // selbst, dessen Regelwerk die gesuchten Muster als Text enthält.
  const tamperIgnore = raw.tamper?.ignore ?? []
  if (!Array.isArray(tamperIgnore) || tamperIgnore.some((p) => typeof p !== 'string')) {
    throw new ConfigError(`${file}: tamper.ignore muss eine Liste von Pfad-Präfixen sein.`)
  }

  return { version, level, baseline, components, configs, tamperIgnore, profile: PROFILE[level] }
}
