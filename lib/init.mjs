...
import { phpstanBinds } from './wiring.mjs'
import { c } from './run.mjs'

/** Dieselbe Liste, die auch der Audit misst — siehe lib/detect.mjs. */
...
export function applyInit(root, recommendation, { pkgRoot, force = false, dryRun = false }) {
  ...
  for (const component of recommendation.components) {
    ...
    if (existsSync(target) && !force) {
      ...
      // Nicht überschreiben, aber auch nicht schweigen: eine bestehende
      // phpstan.neon kann die Basiskonfiguration des Pakets komplett ignorieren.
      // Genau so lief rankscan/application vier Wochen an ihr vorbei, weil hier
      // nur "existiert bereits" stand.
      if (!phpstanBinds(readFileSync(target, 'utf8'))) {
        notes.push(
          `${relative(root, target)} bindet configs/phpstan/base.neon nicht ein — ohne die Basis fehlen ` +
            'reportUnmatchedIgnoredErrors, treatPhpDocTypesAsCertain: false und die excludePaths. ' +
            'Zeile "- vendor/standout/quality/configs/phpstan/base.neon" unter includes: ergänzen, nach Larastan.'
        )
      }
      continue
    }
