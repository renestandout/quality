...
import { componentsForWiring, inspectWiring, ruffBinds } from './wiring.mjs'
...
test('ruff: eine eigene ruff.toml ohne extend ist ein Befund', () => {
  const findings = inspectWiring({
    components: [{ path: 'tools/analyse', stack: 'python' }],
    read: (path) => (path === 'tools/analyse/ruff.toml' ? 'line-length = 100\n[lint]\nselect = ["F"]\n' : null),
  })
  assert.deepEqual(
    findings.map((f) => [f.check, f.status]),
    [['ruff', 'missing']]
  )
})

test('ruff: ein extend auf die Paketdatei zaehlt als Einbindung', () => {
  assert.ok(ruffBinds('extend = "node_modules/@standout/quality/configs/ruff.toml"'))
  assert.ok(ruffBinds('extend = "vendor/standout/quality/configs/ruff.toml"'))
  // Beim Entwickeln am Paket selbst zeigt der Pfad relativ dorthin.
  assert.ok(ruffBinds('extend = "../quality/configs/ruff.toml"'))
  assert.ok(!ruffBinds('# extend = "node_modules/@standout/quality/configs/ruff.toml"'))
  assert.ok(!ruffBinds('line-length = 100'))
})

test('ruff: eine pyproject.toml ohne [tool.ruff] ist keine ruff-Konfiguration', () => {
  // Sonst gaelte jedes Python-Projekt mit pyproject.toml als falsch verdrahtet,
  // auch wenn es ruff gar nicht ueber diese Datei konfiguriert.
  const findings = inspectWiring({
    components: [{ path: '.', stack: 'python' }],
    read: (path) => (path === 'pyproject.toml' ? '[project]\nname = "analyse"\n' : null),
  })
  assert.deepEqual(findings, [])
})

test('ruff: "own" schaltet den Hinweis dauerhaft ab', () => {
  const findings = inspectWiring({
    components: [{ path: '.', stack: 'python' }],
    read: (path) => (path === 'ruff.toml' ? 'line-length = 79\n' : null),
    configs: { ruff: 'own' },
  })
  assert.deepEqual(findings, [])
})
