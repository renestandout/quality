...
test('eine bestehende phpstan.neon ohne die Paket-Basis wird als solche benannt', () => {
  // Der Vorfall in rankscan/application: init sagte nur "existiert bereits", und
  // dass die Datei die Basiskonfiguration nie eingebunden hat, stand nirgends.
  const root = fixture()
  try {
    writeFileSync(join(root, 'backend', 'phpstan.neon'), 'parameters:\n    level: 5\n')
    const result = applyInit(root, RECOMMENDATION, { pkgRoot: PKG_ROOT })
    assert.ok(result.skipped.some((s) => s.file === 'backend/phpstan.neon'))
    assert.ok(
      result.notes.some((n) => /backend\/phpstan\.neon/.test(n) && /base\.neon/.test(n)),
      `erwartet wurde ein Hinweis auf die fehlende Einbindung, bekommen: ${JSON.stringify(result.notes)}`
    )
  } finally {
    rmSync(root, { recursive: true, force: true })
  }
...
test('eine bestehende phpstan.neon MIT der Paket-Basis erzeugt keinen Hinweis', () => {
  const root = fixture()
  try {
    writeFileSync(
      join(root, 'backend', 'phpstan.neon'),
      'includes:\n    - vendor/standout/quality/configs/phpstan/base.neon\n'
    )
    const result = applyInit(root, RECOMMENDATION, { pkgRoot: PKG_ROOT })
    assert.ok(!result.notes.some((n) => /base\.neon/.test(n)))
  } finally {
    rmSync(root, { recursive: true, force: true })
  }
})
