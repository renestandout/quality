...
import { COMMANDS, ONLY_NAMES, STAGES, parseArgs, planFor } from './args.mjs'
...
test('prune ist ein Kommando, keine Prüfstufe', () => {
  // Es ändert eine Datei und läuft darum nicht in einer Stufe mit, die nur prüft.
  assert.ok(COMMANDS.includes('prune'))
  assert.ok(!Object.keys(STAGES).includes('prune'))
})

test('prune nimmt --dry-run', () => {
  assert.equal(parseArgs(['prune', '--dry-run']).stage, 'prune')
  assert.equal(parseArgs(['prune', '--dry-run']).dryRun, true)
  assert.equal(parseArgs(['prune']).dryRun, false)
})
