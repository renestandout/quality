...
function severityLine(counts) {
  ...
  for (const [key, label] of [
    ...
    ['moderate', 'mittel'],
    ['low', 'niedrig'],
    // pip-audit liefert keinen Schweregrad. "ohne Einstufung" sagt das,
    // statt die Funde still als niedrig auszugeben.
    ['unknown', 'ohne Einstufung'],
  ]) {
...
export function renderAudit(report, { generatedAt } = {}) {
  ...
  for (const entry of report.components) {
    ...
    if (entry.deps) {
      // Womit gemessen wurde, gehört dazu: npm, pnpm und composer kommen bei
      // demselben Projekt auf verschiedene Zahlen, weil sie verschiedene
      // Abhängigkeitsbäume sehen.
      const measured = entry.deps.tool
        ? `${severityLine(entry.deps.counts)} — gemessen mit \`${entry.deps.tool} audit\``
        : severityLine(entry.deps.counts)
      lines.push(
        `- **Abhängigkeiten:** ${entry.deps.available ? measured : `nicht geprüft — ${entry.deps.hint}`}`
      )
    }
