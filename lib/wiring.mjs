...
export function prettierBinds(text) {
  return withoutComments(withoutComments(text, '//'), '#').includes(NPM_PACKAGE)
}
...
const extendsBindsPackage = (value) => {
  ...
  return list.some((entry) => typeof entry === 'string' && entry.startsWith(NPM_PACKAGE))
}
      ...
      if (configs.prettier !== 'own') {
        ...
        if (found && !prettierBinds(found.text)) {
          add(
            ...
            'missing',
            found.file,
            `erweitert ${NPM_PACKAGE}/prettier nicht. Ist der Stil bewusst eigen, ` +
          ...
          if (config && !extendsBindsPackage(config.extends)) {
            ...
            if (!viaReference) {
              add(
                ...
                'missing',
                found.file,
                `erweitert ${NPM_PACKAGE}/tsconfig nicht (oder -tsconfig-strict). ` +
