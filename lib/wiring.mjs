 ...
 * nicht von einer eigenen Konfiguration unterscheiden. Für mypy gilt
 * dasselbe — auch `mypy.ini` kennt weder extend noch include. Geprüft wird
 * im Python-Stack deshalb nur ruff, das `extend` beherrscht.
 *
...
const PY_STACKS = ['python']

/** Die Basiskonfiguration im Python-Ökosystem, relativ zur Installation. */
const RUFF_BASE = 'configs/ruff.toml'

/** Kandidaten in der Reihenfolge, in der ruff selbst sucht. */
const RUFF_FILES = ['ruff.toml', '.ruff.toml', 'pyproject.toml']
...
/**
 * Bindet die Datei die ruff-Basis ein?
 *
 * Geprüft wird auf den Dateipfad statt auf den Paketnamen: ruff kennt keine
 * npm-Auflösung, `extend` nimmt nur einen Pfad. Ein Python-Projekt zeigt
 * deshalb auf `node_modules/@standout/quality/configs/ruff.toml` oder auf
 * die Composer-Installation — beides ist dieselbe Datei.
 *
 * `;` und `#` sind beide Kommentarzeichen in TOML-nahen Dateien; geprüft
 * wird gegen `#`, das TOML tatsächlich verwendet.
 */
export function ruffBinds(text) {
  return withoutComments(text, '#').includes(RUFF_BASE)
}
...
export function inspectWiring({ components, read, configs = {} }) {
  ...
  for (const component of components) {
    ...
    if (PY_STACKS.includes(component.stack) && configs.ruff !== 'own') {
      const found = first(RUFF_FILES)
      // Eine pyproject.toml ohne [tool.ruff] sagt nichts über die Einbindung —
      // sie ist dann schlicht keine ruff-Konfiguration.
      const isRuffConfig = found && (!found.file.endsWith('pyproject.toml') || found.text.includes('[tool.ruff'))
      if (found && isRuffConfig && !ruffBinds(found.text)) {
        add(
          'ruff',
          'missing',
          found.file,
          `bindet ${RUFF_BASE} nicht ein — ohne die Basis gilt der Minimalregelsatz (E4, E7, E9, F). ` +
            'Zeile `extend = "node_modules/@standout/quality/configs/ruff.toml"` ergänzen. ' +
            'Ist die Auswahl bewusst eigen, schaltet "configs: { ruff: own }" in quality.yml diesen Hinweis ab.'
        )
      }
    }
