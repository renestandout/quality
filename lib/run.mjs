...
export function runStep(step, { quiet = false } = {}) {
  return new Promise((resolvePromise) => {
    if (step.skip) {
      resolvePromise({ name: step.name, soft: step.soft, code: 0, skipped: true, reason: step.skip, output: '' })
      return
    }
    ...
    child.on('error', (err) => {
      resolvePromise({ name: step.name, note: step.note, soft: step.soft, code: 127, output: `${output}\n${err.message}`, spawnError: true })
    })
    child.on('close', (code) => {
      resolvePromise({ name: step.name, note: step.note, soft: step.soft, code: code ?? 1, output })
    })
  })
}
...
export async function runSteps(steps, { quiet = false, stopOnFail = false } = {}) {
  ...
  for (const step of steps) {
    // Der Marker steht vor der Ausgabe, nicht nur in der Zusammenfassung: wer
    // das Protokoll liest, soll den Befund sofort einordnen können und nicht
    // erst am Ende erfahren, dass er nicht blockiert.
    if (!quiet) process.stdout.write(c.dim(`\n▸ ${step.name}${step.soft ? ' — Hinweis, blockiert nicht' : ''}\n`))
...
/**
 * Ein weicher Schritt ist keine Prüfung, sondern ein Hinweis: er zählt weder
 * in der Summe mit noch entscheidet er über den Exit-Code. Sichtbar wird er
 * nur, wenn er etwas gefunden hat — ein Hinweis ohne Inhalt ist eine Zeile,
 * die niemand braucht.
 */
export function summarize(results, { label }) {
  const failed = results.filter((r) => r.code !== 0 && !r.soft)
  const soft = results.filter((r) => r.soft)
  const skipped = results.filter((r) => r.skipped && !r.soft)
  const ran = results.length - skipped.length - soft.length

  process.stdout.write('\n' + c.bold(`── ${label} ──`) + '\n')
  for (const r of results) {
    if (r.soft) {
      // Übersprungen heisst hier: fehlt das Werkzeug, sagt das schon der harte
      // Lauf. Dieselbe Zeile ein zweites Mal ist keine zweite Auskunft.
      if (!r.skipped && r.code !== 0) process.stdout.write(`  ${c.yellow('!')} ${r.name}${note(r)}\n`)
    } else if (r.skipped) {
