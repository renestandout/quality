...
export function runStep(step, { quiet = false } = {}) {
  return new Promise((resolvePromise) => {
    ...
    child.on('error', (err) => {
      resolvePromise({ name: step.name, code: 127, output: `${output}\n${err.message}`, spawnError: true })
    })
    child.on('close', (code) => {
      resolvePromise({ name: step.name, code: code ?? 1, output })
    })
  })
}
...
export function summarize(results, { label }) {
  ...
  for (const r of results) {
    if (r.skipped) {
      ...
      process.stdout.write(`  ${c.green('✓')} ${r.name}\n`)
    } else {
      process.stdout.write(`  ${c.red('✗')} ${r.name} ${c.dim(`(exit ${r.code})`)}\n`)
    }
  }
  if (failed.length === 0) {
